...
export const handleDocument = () => {
  ...
  return {
    ...
    cardsContainer: data?.body?.map((prismicCardContainer) => ({
      ...
      description:
        prismicCardContainer?.primary?.card_container_description?.[0].text,
      showAsCarousel:
        prismicCardContainer?.primary?.card_container_is_carrousel,
      cards: prismicCardContainer?.items?.map((prismicCard) => ({
        ...
        image: {
          url:
            prismicCard?.card_image?.recommended?.url ??
            prismicCard?.card_image?.url ??
            "",
          alt:
            prismicCard?.card_image?.recommended?.alt ??
            prismicCard?.card_image?.alt ??
            "",
        },
      })),
    })),
